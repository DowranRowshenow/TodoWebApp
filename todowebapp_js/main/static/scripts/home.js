...
// const item_list = [];
var is_active = false;
var current_items_list = item_list;
var current_items = [];
var settings = {
    saved_items: [],
    is_loaded: false,
...
function settings_reader()
{
    ...
    console.log("saved_items: " + settings.saved_items.length);
    ...
    _settings = null;
    current_items_list = settings.saved_items.concat(current_items_list);
    update_items(current_items_list);
    console.log("\nSETTINGS: " + settings);
    console.log("sort_by: " + settings.sort_by);
    console.log("filter_by: " + settings.filter_by);
    console.log("is_loaded: " + settings.is_loaded);
...
function ui_values()
{
    ...
    counter.innerText += current_items_list.length;
...
function update_items(update_items)
{
    ...
    for (i=0; i<update_items.length; i++)
    {
        ...
        {
            let recover = document.createElement('a');
            recover.className = "favorite";
            recover.href = "/home/recover_item/" + update_items[i].id;
            recover.innerText = "Recover Task";
            options.appendChild(recover);
            current_items.push(item);
            
            let deleted = document.createElement('a');
            deleted.className = "delete";
            deleted.href = "/home/delete_item/" + update_items[i].id;
        ...
        else
        {
            ...
            favorite.href = "/home/favorite_item/" + update_items[i].id;
            if (update_items[i].is_favorite == "True") 
            ...
            complete.href = "/home/complete_item/" + update_items[i].id;
            if (update_items[i].is_completed == "True") 
